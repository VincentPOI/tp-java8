...
public class Lambda_01_Test {
	...
	public void test_filter_by_age() throws Exception {
		...
		PersonPredicate sup18Predicate = (Person p) -> p.getAge() >= 18;
		List<Person> result = filter(personList, sup18Predicate);
	...
	public void test_filter_by_firstname() throws Exception {
		...
		PersonPredicate namePredicate = (Person p) -> p.getFirstname().equals("first_10");
		List<Person> result = filter(personList, namePredicate);
	...
	public void test_filter_by_password() throws Exception {
		...
		PersonPredicate passwordPredicate = (
		...
		List<Person> result = filter(personList, passwordPredicate);
